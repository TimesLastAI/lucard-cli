...
use lucard_app::dto::ToolsOverview;
use lucard_app::{
    AgentProviderResolver, AgentRegistry, AppConfigService, AuthService, CommandInfra,
    CommandLoaderService, ConversationService, DataGenerationApp, EnvironmentInfra,
    EnvironmentService, FileDiscoveryService, McpConfigManager, McpService, LucardApp,
    ProviderAuthService, ProviderService, Services, User, UserUsage, Walker,
};
use lucard_common::stream::MpscStream;
use lucard_domain::{Agent, InitAuth, LoginInfo, *};
use lucard_infra::LucardInfra;
use lucard_repo::LucardRepo;
use lucard_services::LucardServices;
use url::Url;

use crate::API;

pub struct LucardAPI<S, F> {
    services: Arc<S>,
    infra: Arc<F>,
}

impl<A, F> LucardAPI<A, F> {
    pub fn new(services: Arc<A>, infra: Arc<F>) -> Self {
        Self { services, infra }
    }

    /// Creates a LucardApp instance with the current services
    fn app(&self) -> LucardApp<A>
    where
        A: Services,
    {
        LucardApp::new(self.services.clone())
    }
}

impl LucardAPI<LucardServices<LucardRepo<LucardInfra>>, LucardRepo<LucardInfra>> {
    pub fn init(restricted: bool, cwd: PathBuf) -> Self {
        let infra = Arc::new(LucardInfra::new(restricted, cwd));
        let repo = Arc::new(LucardRepo::new(infra.clone()));
        let app = Arc::new(LucardServices::new(repo.clone()));
        LucardAPI::new(app, repo)
    }
    ...
    for LucardAPI<A, F>
{
    ...
    async fn user_usage(&self) -> Result<Option<UserUsage>> {
        ...
        if let Some(api_key) = provider
            ...
            .and_then(|c| match &c.auth_details {
                lucard_domain::AuthDetails::ApiKey(key) => Some(key.as_str()),
                _ => None,
            })
        {
    ...
    async fn generate_command(&self, prompt: UserPrompt) -> Result<String> {
        use lucard_app::CommandGenerator;
    ...
    async fn migrate_env_credentials(&self) -> Result<Option<lucard_domain::MigrationResult>> {
        self.services
            .provider_service()
