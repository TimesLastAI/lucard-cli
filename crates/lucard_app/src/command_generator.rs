...
use anyhow::Result;
use lucard_domain::{extract_tag_content, *};
    ...
    S: EnvironmentService + FileDiscoveryService + ProviderService + AppConfigService,
{
    ...
    pub async fn generate(&self, prompt: UserPrompt) -> Result<String> {
        ...
        let rendered_system_prompt = TemplateEngine::default().render(
            "lucard-command-generator-prompt.md",
...
mod tests {
    ...
    impl ProviderService for MockServices {
        ...
        async fn models(&self, _provider: Provider<Url>) -> Result<Vec<lucard_domain::Model>> {
            Ok(vec![])
        }
        ...
        async fn get_all_providers(&self) -> Result<Vec<lucard_domain::AnyProvider>> {
            Ok(vec![])
        }
