...
impl Compactor {
    ...
    fn compress_single_sequence(
        ...
        let summary = TemplateEngine::default().render(
            "lucard-partial-summary-frame.md",
        ...
        if let Some(reasoning) = reasoning_details
            && let Some(ContextMessage::Text(msg)) = context
                .messages
                .iter_mut()
                .find(|msg| msg.has_role(lucard_domain::Role::Assistant))
                .map(|msg| &mut **msg)
            && msg
                .reasoning_details
                .as_ref()
...
mod tests {
    ...
    use lucard_domain::MessageEntry;
    ...
    fn test_compress_single_sequence_preserves_only_last_reasoning() {
        use lucard_domain::ReasoningFull;
    ...
    fn render_template(data: &serde_json::Value) -> String {
        TemplateEngine::default()
            .render("lucard-partial-summary-frame.md", data)
            .unwrap()
    }
    ...
    async fn test_render_summary_frame_snapshot() {
        ...
        let fixture_json = lucard_common::fixture!("/src/fixtures/conversation.json").await;

        let conversation: lucard_domain::Conversation =
        ...
        let environment = test_environment().cwd(PathBuf::from(
            "/Users/tushar/Documents/Projects/code-lucard-workspace/code-lucard",
        ));
        let compactor = Compactor::new(Compact::new(), environment);
    ...
    fn test_compaction_preserves_usage_information() {
        use lucard_domain::{TokenCount, Usage};
