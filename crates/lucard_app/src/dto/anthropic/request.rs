...
use lucard_domain::{ContextMessage, Image};
...
impl TryFrom<lucard_domain::Context> for Request {
    type Error = anyhow::Error;
    fn try_from(request: lucard_domain::Context) -> std::result::Result<Self, Self::Error> {
        let system_messages = request
            .messages
            .iter()
            .filter_map(|msg| match &**msg {
                ContextMessage::Text(msg) if msg.has_role(lucard_domain::Role::System) => {
                    Some(SystemMessage {
        ...
        Ok(Self {
            messages: request
                .messages
                .into_iter()
                .filter(|message| !message.has_role(lucard_domain::Role::System))
        ...
        Ok(match value {
            ContextMessage::Text(chat_message) => {
                ...
                match chat_message.role {
                    lucard_domain::Role::User => Message { role: Role::User, content },
                    lucard_domain::Role::Assistant => Message { role: Role::Assistant, content },
                    lucard_domain::Role::System => {
                        ...
                        return Err(
                            lucard_domain::Error::UnsupportedRole("System".to_string()).into()
                        );
                    }
                }
            }
...
impl TryFrom<lucard_domain::ToolCallFull> for Content {
    type Error = anyhow::Error;
    fn try_from(value: lucard_domain::ToolCallFull) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(lucard_domain::Error::ToolCallMissingId)?;
...
impl TryFrom<lucard_domain::ToolResult> for Content {
    type Error = anyhow::Error;
    fn try_from(value: lucard_domain::ToolResult) -> std::result::Result<Self, Self::Error> {
        let call_id = value
            .call_id
            .as_ref()
            .ok_or(lucard_domain::Error::ToolCallMissingId)?;
...
impl From<lucard_domain::ToolChoice> for ToolChoice {
    fn from(value: lucard_domain::ToolChoice) -> Self {
        match value {
            lucard_domain::ToolChoice::Auto => ToolChoice::Auto { disable_parallel_tool_use: None },
            lucard_domain::ToolChoice::Call(tool_name) => {
                ToolChoice::Tool { name: tool_name.to_string(), disable_parallel_tool_use: None }
            }
            lucard_domain::ToolChoice::Required => {
                ToolChoice::Any { disable_parallel_tool_use: None }
            }
            lucard_domain::ToolChoice::None => ToolChoice::Auto { disable_parallel_tool_use: None },
        }
    }
}
...
impl TryFrom<lucard_domain::ToolDefinition> for ToolDefinition {
    type Error = anyhow::Error;
    fn try_from(value: lucard_domain::ToolDefinition) -> std::result::Result<Self, Self::Error> {
        Ok(ToolDefinition {
            name: value.name.to_string(),
