...
use lucard_domain::ContextMessage;
...
impl TryFrom<lucard_domain::Context> for Request {
    ...
    fn try_from(context: lucard_domain::Context) -> Result<Self, Self::Error> {
        // Extract system message as system_instruction
        let system_instruction = context
            .messages
            .iter()
            .filter_map(|msg| match &**msg {
                ContextMessage::Text(text_msg) if text_msg.has_role(lucard_domain::Role::System) => {
                    Some(Content {
        ...
        let contents = context
            .messages
            .iter()
            .filter(|msg| !msg.has_role(lucard_domain::Role::System))
        ...
        let tool_config = context.tool_choice.map(|tc| {
            let mode = match tc {
                lucard_domain::ToolChoice::Auto => FunctionCallingMode::Auto,
                lucard_domain::ToolChoice::None => FunctionCallingMode::None,
                lucard_domain::ToolChoice::Required => FunctionCallingMode::Any,
                lucard_domain::ToolChoice::Call(name) => {
                    return ToolConfig {
        ...
        match msg {
            ContextMessage::Text(text_msg) => {
                let role = match text_msg.role {
                    lucard_domain::Role::User => Role::User,
                    lucard_domain::Role::Assistant => Role::Model,
