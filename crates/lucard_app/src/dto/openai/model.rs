    ...
    fn from(value: Model) -> Self {
        ...
        let input_modalities = value
            ...
            .map(|modalities| {
                modalities
                    .iter()
                    .filter_map(|s| s.parse::<lucard_domain::InputModality>().ok())
                    .collect::<Vec<_>>()
            })
            .unwrap_or_else(|| vec![lucard_domain::InputModality::Text]);
...
mod tests {
    ...
    async fn test_model_conversion_without_supported_parameters() {
        ...
        let domain_model: lucard_domain::Model = model.into();
