...
impl From<ReasoningDetail> for lucard_domain::ReasoningDetail {
    fn from(detail: ReasoningDetail) -> Self {
        lucard_domain::ReasoningDetail {
            text: detail.text,
...
impl GitHubCopilotReasoning {
    fn into_reasoning_detail(self) -> lucard_domain::ReasoningDetail {
        lucard_domain::ReasoningDetail {
            text: self.text,
            data: self.data,
            type_of: self.r#type,
                    ...
                    let mut response = match choice {
                        ...
                        Choice::NonStreaming { message, finish_reason, .. } => {
                            ...
                            if let Some(reasoning_details) = &message.reasoning_details {
                                let converted_details: Vec<lucard_domain::ReasoningFull> =
                                    reasoning_details
                                        .clone()
                                        .into_iter()
                                        .map(lucard_domain::ReasoningFull::from)
                                        .collect();

                                resp = resp.add_reasoning_detail(lucard_domain::Reasoning::Full(
                                    converted_details,
                                ));
                                ...
                                for tool_call in tool_calls {
                                    resp = resp.add_tool_call(ToolCallFull {
                                        ...
                                        name: tool_call
                                            ...
                                            .name
                                            .clone()
                                            .ok_or(lucard_domain::Error::ToolCallMissingName)?,
                        ...
                        Choice::Streaming { delta, finish_reason, .. } => {
                            ...
                            if let Some(reasoning_details) = &delta.reasoning_details {
                                let converted_details: Vec<lucard_domain::ReasoningPart> =
                                    reasoning_details
                                        .clone()
                                        .into_iter()
                                        .map(lucard_domain::ReasoningPart::from)
                                        .collect();
                                resp = resp.add_reasoning_detail(lucard_domain::Reasoning::Part(
                                    converted_details,
                                ));
                            } else if let Some(details) = convert_github_copilot_reasoning(
...
mod tests {
    use anyhow::Context;
    use lucard_domain::ChatCompletionMessage;
    ...
    async fn test_responses() -> anyhow::Result<()> {
        let content = lucard_common::fixture!("/src/dto/openai/responses.jsonl").await;
