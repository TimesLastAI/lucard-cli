use lucard_domain::{DefaultTransformation, Provider, ProviderId, Transformer};
use url::Url;
...
mod tests {
    ...
    fn lucard(key: &str) -> Provider<Url> {
        Provider {
            id: ProviderId::FORGE,
    ...
    fn test_supports_open_router_params() {
        assert!(supports_open_router_params(&lucard("lucard")));
    ...
    fn test_zai_provider_applies_thinking_transformation() {
        ...
        let fixture = Request::default().reasoning(lucard_domain::ReasoningConfig {
            enabled: Some(true),
            effort: None,
            max_tokens: None,
            exclude: None,
