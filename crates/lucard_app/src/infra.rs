...
/// Environment exposes:
/// - Global agent directory via `agent_path()` (typically ~/.lucard/agents)
...
/// This uses the EnvironmentService trait from lucard_domain
...
pub trait AuthStrategy: Send + Sync {
    ...
    async fn init(&self) -> anyhow::Result<lucard_domain::AuthContextRequest>;

    /// Complete authentication flow
    async fn complete(
        &self,
        context_response: lucard_domain::AuthContextResponse,
    ) -> anyhow::Result<lucard_domain::AuthCredential>;
