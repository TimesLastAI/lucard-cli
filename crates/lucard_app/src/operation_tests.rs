...
use lucard_domain::{Environment, FSRead, Metrics, ToolKind, ToolValue};
...
fn to_value(output: lucard_domain::ToolOutput) -> String {
...
fn test_shell_output_no_truncation() {
    let fixture = ToolOperation::Shell {
        output: ShellOutput {
            output: lucard_domain::CommandOutput {
...
fn test_fs_remove_success() {
    let fixture = ToolOperation::FsRemove {
        input: lucard_domain::FSRemove { path: "/home/user/file_to_delete.txt".to_string() },
...
fn test_fs_patch_basic() {
    ...
    let fixture = ToolOperation::FsPatch {
        input: lucard_domain::FSPatch {
            path: "/home/user/test.txt".to_string(),
            search: Some("world".to_string()),
            operation: lucard_domain::PatchOperation::Replace,
...
fn test_fs_patch_with_warning() {
    ...
    let fixture = ToolOperation::FsPatch {
        input: lucard_domain::FSPatch {
            ...
            operation: lucard_domain::PatchOperation::Append,
            content: "\nnew line".to_string(),
        },
        output: PatchOutput {
            errors: vec![],
...
fn test_fs_undo_no_changes() {
    let fixture = ToolOperation::FsUndo {
        input: lucard_domain::FSUndo { path: "/home/user/unchanged_file.txt".to_string() },
...
fn test_fs_undo_file_created() {
    let fixture = ToolOperation::FsUndo {
        input: lucard_domain::FSUndo { path: "/home/user/new_file.txt".to_string() },
        output: FsUndoOutput {
            before_undo: None,
...
fn test_fs_undo_file_removed() {
    let fixture = ToolOperation::FsUndo {
        input: lucard_domain::FSUndo { path: "/home/user/deleted_file.txt".to_string() },
        output: FsUndoOutput {
...
fn test_fs_undo_file_restored() {
    let fixture = ToolOperation::FsUndo {
        input: lucard_domain::FSUndo { path: "/home/user/restored_file.txt".to_string() },
        output: FsUndoOutput {
...
fn test_fs_undo_success() {
    let fixture = ToolOperation::FsUndo {
        input: lucard_domain::FSUndo { path: "/home/user/test.txt".to_string() },
        output: FsUndoOutput {
...
fn test_net_fetch_success() {
    let fixture = ToolOperation::NetFetch {
        input: lucard_domain::NetFetch { url: "https://example.com".to_string(), raw: Some(false) },
        output: HttpResponse {
...
fn test_net_fetch_truncated() {
    ...
    let truncation_path =
        TempContentFiles::default().stdout(PathBuf::from("/tmp/lucard_fetch_abc123.txt"));
...
fn test_skill_operation() {
    let fixture = ToolOperation::Skill {
        input: lucard_domain::SkillFetch { name: "test-skill".to_string() },
        output: lucard_domain::Skill::new(
            "test-skill",
            "This is a test skill command with instructions",
            "A test skill for demonstration",
        )
        .path("/home/user/.lucard/skills/test-skill")
        .resources(vec![
            PathBuf::from("/home/user/.lucard/skills/test-skill/resource1.txt"),
            PathBuf::from("/home/user/.lucard/skills/test-skill/resource2.md"),
        ]),
    };
