...
impl AgentService for Runner {
    async fn chat_agent(
        &self,
        _id: &lucard_domain::ModelId,
        context: lucard_domain::Context,
        _provider_id: Option<ProviderId>,
    ) -> lucard_domain::ResultStream<ChatCompletionMessage, anyhow::Error> {
    ...
    async fn call(
        &self,
        _: &lucard_domain::Agent,
        _: &lucard_domain::ToolCallContext,
        test_call: lucard_domain::ToolCallFull,
    ) -> lucard_domain::ToolResult {
...
impl TemplateService for Runner {
    ...
    async fn render_template<V: serde::Serialize + Send + Sync>(
        &self,
        template: lucard_domain::Template<V>,
        object: &V,
...
impl AttachmentService for Runner {
    async fn attachments(&self, _url: &str) -> anyhow::Result<Vec<lucard_domain::Attachment>> {
...
impl SkillFetchService for Runner {
    async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<lucard_domain::Skill> {
