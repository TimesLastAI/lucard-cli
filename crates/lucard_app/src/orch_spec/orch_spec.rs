...
async fn test_no_tool_call_start_end_responses_for_agent_tools() {
    // Call an agent tool (using "lucard" which is configured as an agent in the
    // default workflow)
    let agent_tool_call = ToolCallFull::new("lucard")
        .arguments(ToolCallArguments::from(json!({"tasks": ["analyze code"]})));
    let agent_tool_result =
        ToolResult::new("lucard").output(Ok(ToolOutput::text("analysis complete")));
...
async fn test_reasoning_should_be_in_context() {
    ...
    ctx.agent = ctx
        .agent
        .reasoning(ReasoningConfig::default().effort(lucard_domain::Effort::High));
...
async fn test_reasoning_not_supported_when_disabled() {
    ...
    ctx.agent = ctx.agent.reasoning(
        ReasoningConfig::default()
            .effort(lucard_domain::Effort::High)
