...
use lucard_domain::{ChatCompletionMessage, Content, FinishReason, Workflow};
...
async fn test_system_prompt_tool_supported() {
    let mut ctx = TestContext::default()
        ...
        .files(vec![
            lucard_domain::File { path: "/users/john/foo.txt".to_string(), is_dir: false },
            lucard_domain::File { path: "/users/jason/bar.txt".to_string(), is_dir: false },
        ])
        .mock_assistant_responses(vec![
            ChatCompletionMessage::assistant(Content::full("Sure"))
                .finish_reason(FinishReason::Stop),
