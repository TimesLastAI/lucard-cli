...
pub trait ProviderService: Send + Sync {
    ...
    async fn get_provider(&self, id: lucard_domain::ProviderId) -> anyhow::Result<Provider<Url>>;
    async fn get_all_providers(&self) -> anyhow::Result<Vec<AnyProvider>>;
    async fn upsert_credential(
        &self,
        credential: lucard_domain::AuthCredential,
    ) -> anyhow::Result<()>;
    async fn remove_credential(&self, id: &lucard_domain::ProviderId) -> anyhow::Result<()>;
...
pub trait AppConfigService: Send + Sync {
    ...
    async fn set_default_provider(
        &self,
        provider_id: lucard_domain::ProviderId,
    ) -> anyhow::Result<()>;
    ...
    async fn get_provider_model(
        &self,
        provider_id: Option<&lucard_domain::ProviderId>,
...
pub trait WorkflowService {
    ...
    /// If no path is provided, it will try to find lucard.yaml in the current
...
pub trait ImageReadService: Send + Sync {
    ...
    async fn read_image(&self, path: String) -> anyhow::Result<lucard_domain::Image>;
}
...
pub trait AgentRegistry: Send + Sync {
    ...
    async fn get_agents(&self) -> anyhow::Result<Vec<lucard_domain::Agent>>;

    /// Get agent by ID (from registry store)
    async fn get_agent(&self, agent_id: &AgentId) -> anyhow::Result<Option<lucard_domain::Agent>>;
...
pub trait CommandLoaderService: Send + Sync {
    /// Load all command definitions from the lucard/commands directory
    async fn get_commands(&self) -> anyhow::Result<Vec<lucard_domain::Command>>;
}
...
pub trait PolicyService: Send + Sync {
    ...
    async fn check_operation_permission(
        &self,
        operation: &lucard_domain::PermissionOperation,
...
pub trait SkillFetchService: Send + Sync {
    ...
    async fn fetch_skill(&self, skill_name: String) -> anyhow::Result<lucard_domain::Skill>;
    ...
    async fn list_skills(&self) -> anyhow::Result<Vec<lucard_domain::Skill>>;
}
