    ...
    pub async fn add_system_message(
        ...
        let context = if let Some(system_prompt) = &agent.system_prompt {
            ...
            let ctx = SystemContext {
                ...
                skills,
                tool_names: lucard_domain::ToolNames::new(),
            };
            ...
            let non_static_block = TemplateEngine::default()
                .render_template(Template::new("{{> lucard-custom-agent-template.md }}"), &ctx)?;
...
mod tests {
    ...
    use fake::Fake;
    use lucard_domain::{Agent, Environment};
    ...
    impl SkillFetchService for MockSkillFetchService {
        async fn fetch_skill(&self, _skill_name: String) -> anyhow::Result<lucard_domain::Skill> {
            Ok(
                lucard_domain::Skill::new("test_skill", "Test skill", "Test skill description")
                    .path("/skills/test.md"),
            )
        }
    ...
    fn create_test_agent() -> Agent {
        use lucard_domain::{AgentId, ModelId, ProviderId};
        Agent::new(
    ...
    async fn test_system_prompt_adds_context() {
        ...
        let conversation = lucard_domain::Conversation::generate();
