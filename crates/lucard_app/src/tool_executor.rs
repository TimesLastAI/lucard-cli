...
use lucard_domain::{TitleFormat, ToolCallContext, ToolCallFull, ToolCatalog, ToolOutput};
    ...
    async fn dump_operation(&self, operation: &ToolOperation) -> anyhow::Result<TempContentFiles> {
        match operation {
            ToolOperation::NetFetch { input: _, output } => {
                ...
                if is_truncated {
                    files = files.stdout(
                        self.create_temp_file("lucard_fetch_", ".txt", &output.content)
                            .await?,
                    );
                }
            ...
            ToolOperation::Shell { output } => {
                ...
                if stdout_truncated {
                    files = files.stdout(
                        self.create_temp_file("lucard_shell_stdout_", ".txt", &output.output.stdout)
                            .await?,
                    );
                }
                if stderr_truncated {
                    files = files.stderr(
                        self.create_temp_file("lucard_shell_stderr_", ".txt", &output.output.stderr)
                            .await?,
                    );
                }
