...
mod test {
    ...
    async fn test_is_binary_file() -> Result<()> {
        ...
        let (is_text_or_doc, _) = crate::fs::LucardFS::is_binary_path(text_file.path()).await?;
        ...
        let (is_text_or_doc, file_type) =
            crate::fs::LucardFS::is_binary_path(binary_file.path()).await?;
        ...
        let (is_text_or_doc, file_type) =
            crate::fs::LucardFS::is_binary_path(png_file.path()).await?;
        ...
        let (is_text_or_doc, _) = crate::fs::LucardFS::is_binary_path(empty_file.path()).await?;
