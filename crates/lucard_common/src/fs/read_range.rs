...
mod test {
    ...
    async fn test_read_range_utf8() -> Result<()> {
        ...
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 2, 5).await?;
        ...
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 1, 3).await?;
        assert_eq!(result, "Line 1\nLine 2\nLine 3");
        assert_eq!(info.start_line, 1);
        assert_eq!(info.end_line, 3);

        // Test reading to end
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 8, 10).await?;
        assert_eq!(result, "Line 8\nLine 9\nLine 10");
        assert_eq!(info.start_line, 8);
        assert_eq!(info.end_line, 10);

        // Test reading entire file
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 1, 10).await?;
        assert_eq!(result, content);
        assert_eq!(info.start_line, 1);
        assert_eq!(info.end_line, 10);

        // Test single line
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 5, 5).await?;
        assert_eq!(result, "Line 5");
        assert_eq!(info.start_line, 5);
        assert_eq!(info.end_line, 5);

        // Test first line specifically
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 1, 1).await?;
        assert_eq!(result, "Line 1");
        assert_eq!(info.start_line, 1);
        ...
        // Test invalid ranges
        assert!(
            crate::fs::LucardFS::read_range_utf8(file.path(), 8, 5)
                .await
                .is_err()
        );
        assert!(
            crate::fs::LucardFS::read_range_utf8(file.path(), 15, 10)
                .await
                .is_err()
        );
        assert!(
            crate::fs::LucardFS::read_range_utf8(file.path(), 0, 5)
                .await
                .is_err()
        );
    ...
    async fn test_utf8_multi_line_handling() -> Result<()> {
        ...
        let (result, info) = crate::fs::LucardFS::read_range_utf8(file.path(), 2, 3).await?;
    ...
    async fn test_invalid_utf8_handling() -> Result<()> {
        ...
        let result = crate::fs::LucardFS::read_range_utf8(&file.path(), 1, 4).await;
        assert!(result.is_ok());
        Ok(())
    }
    ...
    async fn test_large_file_ranges() -> Result<()> {
        ...
        let (actual_content, actual_info) =
            crate::fs::LucardFS::read_range_utf8(file.path(), 1, 50).await?;
        assert_eq!(actual_content.lines().count(), 50);
        assert_eq!(actual_info, FileInfo::new(1, 50, 5000));

        // Test: range in the middle
        let (actual_content, actual_info) =
            crate::fs::LucardFS::read_range_utf8(file.path(), 2500, 3500).await?;
        ...
        let (actual_content, actual_info) =
            crate::fs::LucardFS::read_range_utf8(file.path(), 4990, 6000).await?;
