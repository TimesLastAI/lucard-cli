...
impl ResultStreamExt<anyhow::Error> for crate::BoxStream<ChatCompletionMessage, anyhow::Error> {
    async fn into_full(
        ...
        while let Some(message) = self.next().await {
            ...
            if !tool_interrupted {
                ...
                if let Some(content_part) = message.content.as_ref() {
                    ...
                    {
                        let cleaned_content =
                            remove_tag_with_prefix(content_part.as_str(), "lucard_");
        ...
        if buffering_started {
            cleaned_content += &remove_tag_with_prefix(content_buffered.as_str(), "lucard_");
        }
        ...
        if tool_interrupted && !content.trim().ends_with("</lucard_tool_call>") {
            if let Some((i, right)) = content.rmatch_indices("</lucard_tool_call>").next() {
                content.truncate(i + right.len());

                // Add a comment for the assistant to signal interruption
                content.push('\n');
                content.push_str("<lucard_feedback>");
                content.push_str(
...
mod tests {
    ...
    async fn test_into_full_xml_tool_call_interruption_captures_final_usage() {
        let xml_content = r#"<lucard_tool_call>
...
{"name": "test_tool", "arguments": {"arg": "value"}}
    ...
    async fn test_streaming_reasoning_followed_by_buffered_content() {
        ...
        let messages = vec![
            ...
            Ok(ChatCompletionMessage::default().content(Content::part("<lucard_tool_call>"))),
    ...
    async fn test_streaming_buffering_logic_with_reasoning_state() {
        ...
        let messages = vec![
            ...
            Ok(ChatCompletionMessage::default().content(Content::part("<lucard_"))), /* Triggers buffering */
        ...
        {
            assert!(content.starts_with("\n"));
            assert!(content.contains("<lucard_tool>content and more"));
        } else {
