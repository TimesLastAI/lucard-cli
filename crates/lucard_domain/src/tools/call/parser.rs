...
fn parse_tool_call(input: &str) -> IResult<&str, ToolCallParsed> {
    ...
    let (input, _) = tag("<lucard_tool_call>").parse(input)?;
    let (input, _) = multispace0(input)?; // Handle whitespace after <lucard_tool_call>

    // Match the tool name tags: <lucard_tool_name>
    let (input, _) = tag("<").parse(input)?;
...
fn find_next_tool_call(input: &str) -> IResult<&str, &str> {
    ...
    let (remaining, _) = take_until("<lucard_tool_call>").parse(input)?;
    Ok((remaining, ""))
}
...
mod tests {
    ...
    impl ToolCallBuilder {
        ...
        fn build_xml(&self) -> String {
            let mut xml = String::from("<lucard_tool_call>");
            ...
            xml.push_str(&format!("</{}></lucard_tool_call>", self.name));
            xml
        }
    ...
    fn test_parse_new_tool_call_format() {
        let input = r#"<lucard_tool_call><search><path>/test/path</path><regex>test</regex></search></lucard_tool_call>"#;
    ...
    fn test_parse_with_newlines() {
        let input = [
            "<lucard_tool_call><foo><p1>",
            "abc",
            "</p1></foo></lucard_tool_call>",
        ]
        .join("\n");
