...
mod tests {
    ...
    fn test_extract_tag_names_with_prefix() {
        let fixture = "<lucard_tool>Something</lucard_tool> <lucard_tool_call>Content</lucard_tool_call> <other>More</other>";
        let actual = remove_tag_with_prefix(fixture, "lucard");
        // Check that both tool tags have been removed, leaving only <other> tag
        assert!(actual.contains("<other>More</other>"));
        assert!(!actual.contains("<lucard_tool>"));
        assert!(!actual.contains("<lucard_tool_call>"));
    }
    ...
    fn test_extract_tag_names_with_prefix_nested() {
        let fixture =
            "<parent><lucard_tool>Inner</lucard_tool><lucard_tool_call>Nested</lucard_tool_call></parent>";
        let actual = remove_tag_with_prefix(fixture, "lucard");
    ...
    fn test_extract_tag_names_with_prefix_duplicates() {
        let fixture =
            "<lucard_tool>First</lucard_tool><other>Middle</other><lucard_tool>Second</lucard_tool>";
        let actual = remove_tag_with_prefix(fixture, "lucard");
    ...
    fn test_extract_tag_names_with_prefix_attributes() {
        let fixture = "<lucard_tool id=\"1\">Content</lucard_tool> <lucard_tool_call class=\"important\">More</lucard_tool_call>";
        let actual = remove_tag_with_prefix(fixture, "lucard");
    ...
    fn test_remove_tag_with_prefix() {
        let fixture = "<lucard_task>Task details</lucard_task> Regular text <lucard_analysis>Analysis details</lucard_analysis>";
        let actual = remove_tag_with_prefix(fixture, "lucard_");
