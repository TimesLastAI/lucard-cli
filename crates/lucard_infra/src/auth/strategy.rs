...
use lucard_app::{AuthStrategy, OAuthHttpProvider, StrategyFactory};
use lucard_domain::{
...
async fn poll_for_tokens(
    device_code: &lucard_domain::DeviceCode,
    config: &OAuthConfig,
    timeout: Duration,
...
pub struct LucardAuthStrategyFactory {}

impl Default for LucardAuthStrategyFactory {
    fn default() -> Self {
        Self::new()
    }
}

impl LucardAuthStrategyFactory {
    pub fn new() -> Self {
        Self {}
    }
}

impl StrategyFactory for LucardAuthStrategyFactory {
    ...
    fn create_auth_strategy(
        ...
        match auth_method {
            lucard_domain::AuthMethod::ApiKey => Ok(AnyAuthStrategy::ApiKey(ApiKeyStrategy::new(
                provider_id,
                required_params,
            ))),
            lucard_domain::AuthMethod::OAuthCode(config) => {
            ...
            lucard_domain::AuthMethod::OAuthDevice(config) => {
...
mod tests {
    ...
    fn test_create_auth_strategy_api_key() {
        let factory = LucardAuthStrategyFactory::new();
