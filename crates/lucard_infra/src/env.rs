...
use lucard_app::EnvironmentInfra;
use lucard_domain::{Environment, ModelId, ProviderId, RetryConfig, TlsBackend, TlsVersion};
use reqwest::Url;

#[derive(Clone)]
pub struct LucardEnvironmentInfra {
    restricted: bool,
    cwd: PathBuf,
}

impl LucardEnvironmentInfra {
    ...
    fn get(&self) -> Environment {
        ...
        Environment {
            ...
            base_path: dirs::home_dir()
                .map(|a| a.join(".lucard"))
                .unwrap_or(PathBuf::from(".").join(".lucard")),
            home: dirs::home_dir(),
            retry_config,
...
impl EnvironmentInfra for LucardEnvironmentInfra {
...
impl_from_env_str_via_from_str! {
    ...
    f32, f64,
    String,
    lucard_domain::TlsBackend,
    lucard_domain::TlsVersion,
}
...
fn resolve_http_config() -> lucard_domain::HttpConfig {
    let mut config = lucard_domain::HttpConfig::default();
...
mod tests {
    ...
    use lucard_domain::{TlsBackend, TlsVersion};
    ...
    fn test_dot_env_loading() {
        ...
        LucardEnvironmentInfra::dot_env(&cwd);
    ...
    fn test_http_config_parsing() {
        ...
        let expected = lucard_domain::HttpConfig::default();
    ...
    fn test_max_search_result_bytes() {
        ...
        // Test default value
        let lucard_env = LucardEnvironmentInfra::new(false, PathBuf::from("/tmp"));
        let environment = lucard_env.get_environment();
    ...
    fn test_auto_open_dump_env_var() {
        ...
        let infra = LucardEnvironmentInfra::new(false, cwd);
    ...
    fn test_max_image_size_env_var() {
        ...
        let infra = LucardEnvironmentInfra::new(false, cwd.path().to_path_buf());
