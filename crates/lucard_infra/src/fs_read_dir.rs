...
use glob::Pattern;
use lucard_app::DirectoryReaderInfra;
use lucard_common::fs::LucardFS;

/// Service for reading multiple files from a directory asynchronously
pub struct LucardDirectoryReaderService;

impl LucardDirectoryReaderService {
    ...
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
        ...
        if !LucardFS::exists(directory) || LucardFS::is_file(directory) {
            return Ok(vec![]);
        }

        // Read directory entries
        let mut dir = LucardFS::read_dir(directory).await?;
    ...
    async fn read_directory_files(
        ...
        let read_tasks = file_paths.into_iter().map(|path| {
            ...
            async move {
                match LucardFS::read_to_string(&path).await {
...
impl DirectoryReaderInfra for LucardDirectoryReaderService {
    async fn list_directory_entries(&self, directory: &Path) -> Result<Vec<(PathBuf, bool)>> {
...
mod tests {
    ...
    async fn test_read_directory_files_with_filter() {
        ...
        let actual = LucardDirectoryReaderService
    ...
    async fn test_read_directory_files_without_filter() {
        ...
        let mut actual = LucardDirectoryReaderService
