...
pub struct LucardFileWriteService;

impl LucardFileWriteService {
    pub fn new() -> Self {
        Self
    }
    ...
    async fn create_parent_dirs(&self, path: &Path) -> anyhow::Result<()> {
        if !lucard_common::fs::LucardFS::exists(path)
            && let Some(parent) = path.parent()
        {
            lucard_common::fs::LucardFS::create_dir_all(parent).await?;
        }
        Ok(())
    }
}

impl Default for LucardFileWriteService {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FileWriterInfra for LucardFileWriteService {
    async fn write(&self, path: &Path, contents: Bytes) -> anyhow::Result<()> {
        self.create_parent_dirs(path).await?;
        Ok(lucard_common::fs::LucardFS::write(path, contents.to_vec()).await?)
    }
...
mod tests {
    ...
    fn create_test_service() -> LucardFileWriteService {
        LucardFileWriteService::new()
    }
