...
use anyhow::Context;
use bytes::Bytes;
use lucard_app::HttpInfra;
use lucard_domain::{Environment, TlsBackend, TlsVersion};
...
impl<F> LucardHttpInfra<F> {
    ...
    fn headers(&self, headers: Option<HeaderMap>) -> HeaderMap {
        ...
        headers.insert("User-Agent", HeaderValue::from_static("Lucard"));
        headers.insert("X-Title", HeaderValue::from_static("lucard"));
        headers.insert(
            "x-app-version",
        ...
        headers.insert(
            "HTTP-Referer",
            HeaderValue::from_static("https://lucardcode.dev"),
        );
        headers.insert(
...
impl<F: lucard_app::FileWriterInfra + 'static> LucardHttpInfra<F> {
...
impl<F: lucard_app::FileWriterInfra + 'static> HttpInfra for LucardHttpInfra<F> {
...
mod tests {
    ...
    use fake::{Fake, Faker};
    use lucard_app::FileWriterInfra;
    use lucard_domain::{Environment, HttpConfig};
    ...
    async fn test_debug_requests_none_does_not_write() {
        ...
        let http = LucardHttpInfra::new(env, Arc::new(file_writer.clone()));
    ...
    async fn test_debug_requests_with_valid_path() {
        let file_writer = MockFileWriter::new();
        let debug_path = PathBuf::from("/tmp/lucard-test/debug.json");
