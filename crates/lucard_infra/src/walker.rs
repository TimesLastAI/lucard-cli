...
pub struct LucardWalkerService;

impl LucardWalkerService {
    pub fn new() -> Self {
        Self
    }

    pub async fn walk(&self, config: Walker) -> Result<Vec<WalkedFile>> {
        // Convert domain config to lucard_common::walker config
        let mut walker = if config.max_depth.is_none()
            ...
            && !config.skip_binary
        {
            lucard_common::walker::Walker::max_all()
        } else {
            lucard_common::walker::Walker::min_all()
        };
...
mod tests {
    ...
    async fn test_walker_service_basic_functionality() {
        ...
        let service = LucardWalkerService::new();
