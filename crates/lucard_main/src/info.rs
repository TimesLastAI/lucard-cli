...
use lucard_api::{Conversation, Environment, LoginInfo, Metrics, Role, Usage, UserUsage};
use lucard_app::utils::truncate_key;
const VERSION: &str = env!("CARGO_PKG_VERSION");

use crate::display_constants::markers;
use crate::model::LucardCommandManager;
...
impl From<&Environment> for Info {
    fn from(env: &Environment) -> Self {
        ...
        info = info
            ...
            .add_key_value("Lucard API URL", env.lucard_api_url.to_string())
...
/// Create an info instance for available commands from a LucardCommandManager
impl From<&LucardCommandManager> for Info {
    fn from(command_manager: &LucardCommandManager) -> Self {
        ...
        info = info
            ...
            .add_key_value("<CTRL+D>", "Quit Lucard interactive shell")
    ...
    fn from(user_usage: &UserUsage) -> Self {
        ...
        if plan.is_upgradeable() {
            info = info.add_key_value(
                "Subscription",
                format!(
                    "{} [Upgrade https://app.lucardcode.dev/app/billing]",
...
pub(crate) fn format_user_message(msg: &lucard_api::ContextMessage) -> Option<String> {
    let content = msg
        .as_value()
...
mod tests {
    ...
    use lucard_api::{Environment, EventValue};
    ...
    fn test_metrics_info_display() {
        use lucard_api::Metrics;
        use lucard_domain::{FileOperation, ToolKind};
    ...
    fn test_conversation_info_display() {
        ...
        let fixture = Conversation {
            ...
            context: None,
            metrics,
            metadata: lucard_domain::MetaData::new(Utc::now()),
        };
    ...
    fn test_conversation_info_display_with_task() {
        use chrono::Utc;
        use lucard_api::{Context, ContextMessage, ConversationId, Role};
        ...
        let context = Context::default()
            ...
            .add_message(ContextMessage::Text(
                lucard_domain::TextMessage::new(Role::User, "First user message")
                    .raw_content(EventValue::text("First user message")),
            ))
            .add_message(ContextMessage::assistant("Assistant response", None, None))
            .add_message(ContextMessage::Text(
                lucard_domain::TextMessage::new(Role::User, "Create a new feature")
    ...
    fn test_info_from_command_manager() {
        let command_manager = super::LucardCommandManager::default();
