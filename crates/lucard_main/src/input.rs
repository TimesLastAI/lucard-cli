...
use lucard_api::Environment;
use serde::{Deserialize, Serialize};

use crate::model::{LucardCommandManager, SlashCommand};
...
impl Console {
    ...
    pub fn new(env: Environment, command: Arc<LucardCommandManager>) -> Self {
        Self { env, command }
    }
...
impl Console {
    pub async fn prompt(&self, prompt: LucardPrompt) -> anyhow::Result<SlashCommand> {
...
mod tests {
    ...
    fn test_load_and_save_history() {
        ...
        let history_path = temp_dir.path().join(".lucard_history");
