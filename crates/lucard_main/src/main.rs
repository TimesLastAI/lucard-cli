...
use anyhow::Result;
use clap::Parser;
use lucard_api::LucardAPI;
use lucard_domain::TitleFormat;
use lucard_main::{Cli, Sandbox, TitleDisplayExt, UI};
...
async fn main() -> Result<()> {
    ...
    // Initialize the LucardAPI with the restricted mode if specified
    let restricted = cli.restricted;
    let mut ui = UI::init(cli, move || LucardAPI::init(restricted, cwd.clone()))?;
    ui.run().await;
...
mod tests {
    ...
    fn test_stdin_detection_logic() {
        ...
        let cli_with_prompt = Cli::parse_from(["lucard", "--prompt", "existing prompt"]);
        ...
        let cli_no_prompt = Cli::parse_from(["lucard"]);
    ...
    fn test_cli_parsing_with_short_flag() {
        ...
        let cli_with_short_prompt = Cli::parse_from(["lucard", "-p", "short flag prompt"]);
        assert_eq!(
    ...
    fn test_cli_parsing_other_flags_work_with_piping() {
        ...
        let cli_with_flags = Cli::parse_from(["lucard", "--verbose", "--restricted"]);
