...
use lucard_api::{AgentId, Conversation, Environment, TextMessage, TokenCount, ToolCatalog};
use lucard_app::fmt::content::FormatContent;
use lucard_common::display::md::render::{MarkdownRenderer, crossterm as md_crossterm};
use lucard_domain::{ChatResponseContent, ContextMessage, Role};
use tokio_stream::StreamExt;

use crate::info::Info;
use crate::prompt::LucardPrompt;
    ...
    pub fn render_summary(&self, conversation: &Conversation) -> Vec<String> {
        ...
        if !conversation.metrics.file_operations.is_empty() {
            ...
            operations.sort_by(|(path_a, op_a), (path_b, op_b)| {
                let get_priority = |op: &lucard_domain::FileOperation| match op.tool {
                    lucard_domain::ToolKind::Remove => 0,
                    lucard_domain::ToolKind::Patch => 1,
                    lucard_domain::ToolKind::Write => 2,
                    lucard_domain::ToolKind::Undo => 3,
                    lucard_domain::ToolKind::Read => 4,
                    _ => 5,
                };
            ...
            for (path, operation) in operations {
                let (op_letter, op_color) = match operation.tool {
                    lucard_domain::ToolKind::Write => ('w', colored::Color::Green),
                    lucard_domain::ToolKind::Patch => ('p', colored::Color::Yellow),
                    lucard_domain::ToolKind::Remove => ('d', colored::Color::Red),
                    lucard_domain::ToolKind::Undo => ('u', colored::Color::Blue),
                    lucard_domain::ToolKind::Read => ('r', colored::Color::Cyan),
