...
use lucard_api::{API, Update};
const VERSION: &str = env!("CARGO_PKG_VERSION");
use update_informer::{Check, Version, registry};

/// Package name for lucard on npm.
const FORGE_NPM_PACKAGE: &str = "lucardcode";
...
async fn execute_update_command(api: Arc<impl API>) {
    ...
    match output {
        ...
        Ok(output) => {
            if output.success() {
                let answer = lucard_common::select::LucardSelect::confirm(
                    "You need to close lucard to complete update. Do you want to close it now?",
                )
                .with_default(true)
                .prompt();
...
async fn confirm_update(version: Version) -> bool {
    let answer = lucard_common::select::LucardSelect::confirm(format!(
