...
mod tests {
    ...
    fn test_glob_pattern_includes_all_directories() {
        ...
        // Should NOT include lucard.plugin.zsh (it's excluded)
        let has_plugin_file = files.iter().any(|f| f == "lucard.plugin.zsh");
        assert!(!has_plugin_file, "Should exclude lucard.plugin.zsh");
