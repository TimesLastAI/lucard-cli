...
use lucard_app::domain::{AgentDefinition, Template};
use lucard_app::{AgentRepository, DirectoryReaderInfra, EnvironmentInfra, FileInfoInfra};

/// Infrastructure implementation for loading agent definitions from multiple
/// sources:
/// 1. Built-in agents (embedded in the application)
/// 2. Global custom agents (from ~/.lucard/agents/ directory)
/// 3. Project-local agents (from .lucard/agents/ directory in current working
///    directory)
///
/// ## Agent Precedence
...
/// - **Global agents**: `{HOME}/.lucard/agents/*.md`
/// - **CWD agents**: `./.lucard/agents/*.md` (relative to current working
///   directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct LucardAgentRepository<I> {
    infra: Arc<I>,
}

impl<I> LucardAgentRepository<I> {
...
impl<I: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra> AgentRepository
    for LucardAgentRepository<I>
{
    /// Load all agent definitions from all available sources with conflict
    /// resolution.
    async fn get_agents(&self) -> anyhow::Result<Vec<lucard_app::domain::AgentDefinition>> {
        self.load_agents().await
    }
}

impl<I: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra> LucardAgentRepository<I> {
    ...
    async fn init_default(&self) -> anyhow::Result<Vec<AgentDefinition>> {
        parse_agent_iter(
            [
                ("lucard", include_str!("agents/lucard.md")),
...
mod tests {
    ...
    async fn test_parse_basic_agent() {
        let content = lucard_common::fixture!("/src/fixtures/agents/basic.md").await;
    ...
    async fn test_parse_advanced_agent() {
        let content = lucard_common::fixture!("/src/fixtures/agents/advanced.md").await;
