...
//! `lucard_domain` counterparts for compile-time safety while keeping the
//! storage layer independent from domain model changes.

use anyhow::Context as _;
use chrono::{DateTime, Utc};
use lucard_domain::{Context, ConversationId};
...
impl From<&lucard_domain::ModelId> for ModelIdRecord {
    fn from(id: &lucard_domain::ModelId) -> Self {
        Self(id.to_string())
    }
}

impl From<ModelIdRecord> for lucard_domain::ModelId {
    fn from(record: ModelIdRecord) -> Self {
        lucard_domain::ModelId::from(record.0)
    }
}
...
impl From<&lucard_domain::Image> for ImageRecord {
    fn from(image: &lucard_domain::Image) -> Self {
        Self {
...
impl From<ImageRecord> for lucard_domain::Image {
    fn from(record: ImageRecord) -> Self {
        lucard_domain::Image::new_base64(
            record
                .url
...
impl From<&lucard_domain::ToolCallId> for ToolCallIdRecord {
    fn from(id: &lucard_domain::ToolCallId) -> Self {
        Self(id.as_str().to_string())
    }
}

impl From<ToolCallIdRecord> for lucard_domain::ToolCallId {
    fn from(record: ToolCallIdRecord) -> Self {
        lucard_domain::ToolCallId::new(record.0)
    }
}
...
impl From<&lucard_domain::ToolCallArguments> for ToolCallArgumentsRecord {
    fn from(args: &lucard_domain::ToolCallArguments) -> Self {
        // Serialize to JSON to capture both Parsed and Unparsed variants
        Self(serde_json::to_value(args).unwrap_or_default())
    }
}

impl From<ToolCallArgumentsRecord> for lucard_domain::ToolCallArguments {
...
impl From<&lucard_domain::ToolName> for ToolNameRecord {
    fn from(name: &lucard_domain::ToolName) -> Self {
        Self(name.to_string())
    }
}

impl From<ToolNameRecord> for lucard_domain::ToolName {
    fn from(record: ToolNameRecord) -> Self {
        lucard_domain::ToolName::new(record.0)
    }
}
...
impl From<&lucard_domain::ToolCallFull> for ToolCallFullRecord {
    fn from(call: &lucard_domain::ToolCallFull) -> Self {
        Self {
...
impl From<ToolCallFullRecord> for lucard_domain::ToolCallFull {
    fn from(record: ToolCallFullRecord) -> Self {
        lucard_domain::ToolCallFull {
...
impl From<&lucard_domain::ReasoningFull> for ReasoningFullRecord {
    fn from(reasoning: &lucard_domain::ReasoningFull) -> Self {
        Self {
...
impl From<ReasoningFullRecord> for lucard_domain::ReasoningFull {
    fn from(record: ReasoningFullRecord) -> Self {
        lucard_domain::ReasoningFull {
            text: record.text,
...
impl From<&lucard_domain::TokenCount> for TokenCountRecord {
    fn from(count: &lucard_domain::TokenCount) -> Self {
        match count {
            lucard_domain::TokenCount::Actual(n) => Self::Actual(*n),
            lucard_domain::TokenCount::Approx(n) => Self::Approx(*n),
        }
    }
}

impl From<TokenCountRecord> for lucard_domain::TokenCount {
...
impl From<&lucard_domain::Usage> for UsageRecord {
    fn from(usage: &lucard_domain::Usage) -> Self {
        Self {
...
impl From<UsageRecord> for lucard_domain::Usage {
...
impl From<&lucard_domain::EventValue> for EventValueRecord {
    fn from(event: &lucard_domain::EventValue) -> Self {
        Self(serde_json::to_value(event).unwrap_or_default())
    }
}

impl TryFrom<EventValueRecord> for lucard_domain::EventValue {
...
impl From<&lucard_domain::Role> for RoleRecord {
    fn from(role: &lucard_domain::Role) -> Self {
        match role {
            lucard_domain::Role::System => Self::System,
            lucard_domain::Role::User => Self::User,
            lucard_domain::Role::Assistant => Self::Assistant,
        }
    }
}

impl From<RoleRecord> for lucard_domain::Role {
...
impl From<&lucard_domain::TextMessage> for TextMessageRecord {
    fn from(msg: &lucard_domain::TextMessage) -> Self {
        Self {
...
impl TryFrom<TextMessageRecord> for lucard_domain::TextMessage {
    type Error = anyhow::Error;

    fn try_from(record: TextMessageRecord) -> anyhow::Result<Self> {
        Ok(lucard_domain::TextMessage {
...
impl From<&lucard_domain::ToolValue> for ToolValueRecord {
    fn from(value: &lucard_domain::ToolValue) -> Self {
        match value {
            lucard_domain::ToolValue::Text(text) => Self::Text(text.clone()),
            lucard_domain::ToolValue::AI { value, conversation_id } => Self::AI {
                value: value.clone(),
                conversation_id: conversation_id.into_string(),
            },
            lucard_domain::ToolValue::Image(img) => Self::Image(ImageRecord::from(img)),
            lucard_domain::ToolValue::Empty => Self::Empty,
        }
    }
}

impl TryFrom<ToolValueRecord> for lucard_domain::ToolValue {
    type Error = anyhow::Error;
...
impl From<&lucard_domain::ToolOutput> for ToolOutputRecord {
    fn from(output: &lucard_domain::ToolOutput) -> Self {
        Self {
...
impl TryFrom<ToolOutputRecord> for lucard_domain::ToolOutput {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ToolOutputRecord) -> anyhow::Result<Self> {
        ...
        Ok(lucard_domain::ToolOutput { is_error: record.is_error, values: values? })
    }
}
...
impl From<&lucard_domain::ToolResult> for ToolResultRecord {
    fn from(result: &lucard_domain::ToolResult) -> Self {
        Self {
...
impl TryFrom<ToolResultRecord> for lucard_domain::ToolResult {
    type Error = anyhow::Error;

    fn try_from(record: ToolResultRecord) -> anyhow::Result<Self> {
        Ok(lucard_domain::ToolResult {
            name: record.name.into(),
            call_id: record.call_id.map(Into::into),
...
impl From<&lucard_domain::ContextMessage> for ContextMessageValueRecord {
    fn from(value: &lucard_domain::ContextMessage) -> Self {
        match value {
            lucard_domain::ContextMessage::Text(msg) => Self::Text(TextMessageRecord::from(msg)),
            lucard_domain::ContextMessage::Tool(result) => Self::Tool(ToolResultRecord::from(result)),
            lucard_domain::ContextMessage::Image(img) => Self::Image(ImageRecord::from(img)),
        }
    }
}

impl TryFrom<ContextMessageValueRecord> for lucard_domain::ContextMessage {
    type Error = anyhow::Error;
...
impl From<&lucard_domain::MessageEntry> for ContextMessageRecord {
    fn from(msg: &lucard_domain::MessageEntry) -> Self {
        Self {
...
impl TryFrom<ContextMessageRecord> for lucard_domain::MessageEntry {
    type Error = anyhow::Error;

    fn try_from(record: ContextMessageRecord) -> anyhow::Result<Self> {
        Ok(lucard_domain::MessageEntry {
...
impl From<&lucard_domain::ToolDefinition> for ToolDefinitionRecord {
    fn from(def: &lucard_domain::ToolDefinition) -> Self {
        Self {
...
impl TryFrom<ToolDefinitionRecord> for lucard_domain::ToolDefinition {
    type Error = anyhow::Error;

    fn try_from(record: ToolDefinitionRecord) -> anyhow::Result<Self> {
        Ok(lucard_domain::ToolDefinition {
            name: record.name.into(),
...
impl From<&lucard_domain::ToolChoice> for ToolChoiceRecord {
    fn from(choice: &lucard_domain::ToolChoice) -> Self {
        match choice {
            lucard_domain::ToolChoice::None => Self::None,
            lucard_domain::ToolChoice::Auto => Self::Auto,
            lucard_domain::ToolChoice::Required => Self::Required,
            lucard_domain::ToolChoice::Call(name) => Self::Call(ToolNameRecord::from(name)),
        }
    }
}

impl From<ToolChoiceRecord> for lucard_domain::ToolChoice {
...
impl From<&lucard_domain::Effort> for EffortRecord {
    fn from(effort: &lucard_domain::Effort) -> Self {
        match effort {
            lucard_domain::Effort::High => Self::High,
            lucard_domain::Effort::Medium => Self::Medium,
            lucard_domain::Effort::Low => Self::Low,
        }
    }
}

impl From<EffortRecord> for lucard_domain::Effort {
...
impl From<&lucard_domain::ReasoningConfig> for ReasoningConfigRecord {
    fn from(config: &lucard_domain::ReasoningConfig) -> Self {
        Self {
...
impl From<ReasoningConfigRecord> for lucard_domain::ReasoningConfig {
    fn from(record: ReasoningConfigRecord) -> Self {
        lucard_domain::ReasoningConfig {
...
impl TryFrom<ContextRecord> for Context {
    ...
    fn try_from(record: ContextRecord) -> anyhow::Result<Self> {
        ...
        Ok(Context {
            ...
            temperature: record
                .temperature
                .map(lucard_domain::Temperature::new_unchecked),
            top_p: record.top_p.map(lucard_domain::TopP::new_unchecked),
            top_k: record.top_k.map(lucard_domain::TopK::new_unchecked),
...
pub(super) struct FileChangeMetricsRecord {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    tool: Option<lucard_domain::ToolKind>,
}

impl From<&lucard_domain::FileOperation> for FileChangeMetricsRecord {
    fn from(metrics: &lucard_domain::FileOperation) -> Self {
        Self {
...
impl From<FileChangeMetricsRecord> for lucard_domain::FileOperation {
    fn from(record: FileChangeMetricsRecord) -> Self {
        // Use Write as default tool for old records without tool field
        let tool = record.tool.unwrap_or(lucard_domain::ToolKind::Write);
        Self::new(tool)
...
impl From<&lucard_domain::Metrics> for MetricsRecord {
    fn from(metrics: &lucard_domain::Metrics) -> Self {
        Self {
...
impl From<MetricsRecord> for lucard_domain::Metrics {
...
impl ConversationRecord {
    ...
    pub fn new(
        conversation: lucard_domain::Conversation,
        workspace_id: lucard_domain::WorkspaceHash,
    ) -> Self {
...
impl TryFrom<ConversationRecord> for lucard_domain::Conversation {
    type Error = anyhow::Error;
    ...
    fn try_from(record: ConversationRecord) -> anyhow::Result<Self> {
        ...
        let metrics = record
            ...
            .map(lucard_domain::Metrics::from)
            .unwrap_or_else(|| {
                lucard_domain::Metrics::default().started_at(record.created_at.and_utc())
            });

        Ok(lucard_domain::Conversation::new(id)
            .context(context)
            .title(record.title)
            .metrics(metrics)
            .metadata(
                lucard_domain::MetaData::new(record.created_at.and_utc())
