...
// Re-export CacacheStorage from lucard_infra
pub use lucard_infra::CacacheStorage;
use reqwest::header::HeaderMap;
use reqwest::Response;
use reqwest_eventsource::EventSource;
use url::Url;

use crate::agent::LucardAgentRepository;
...
use crate::fs_snap::LucardFileSnapshotService;
use crate::provider::LucardProviderRepository;
use crate::skill::LucardSkillRepository;
...
#[derive(Clone)]
pub struct LucardRepo<F> {
    infra: Arc<F>,
    file_snapshot_service: Arc<LucardFileSnapshotService>,
    ...
    provider_repository: Arc<LucardProviderRepository<F>>,
    agent_repository: Arc<LucardAgentRepository<F>>,
    skill_repository: Arc<LucardSkillRepository<F>>,
}

impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra> LucardRepo<F> {
    pub fn new(infra: Arc<F>) -> Self {
        let env = infra.get_environment();
        let file_snapshot_service = Arc::new(LucardFileSnapshotService::new(env.clone()));
        let db_pool =
        ...
        let provider_repository = Arc::new(LucardProviderRepository::new(infra.clone()));

        let agent_repository = Arc::new(LucardAgentRepository::new(infra.clone()));
        let skill_repository = Arc::new(LucardSkillRepository::new(infra.clone()));
        Self {
            infra,
...
impl<F: Send + Sync> SnapshotRepository for LucardRepo<F> {
...
#[async_trait::async_trait]
impl<F: Send + Sync> ConversationRepository for LucardRepo<F> {
...
#[async_trait::async_trait]
impl<F: Send + Sync> KVStore for LucardRepo<F> {
...
#[async_trait::async_trait]
impl<F: HttpInfra> HttpInfra for LucardRepo<F> {
...
#[async_trait::async_trait]
impl<F: EnvironmentInfra> EnvironmentInfra for LucardRepo<F> {
...
#[async_trait::async_trait]
impl<F> FileReaderInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> WalkerInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileWriterInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileInfoInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileDirectoryInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> FileRemoverInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> DirectoryReaderInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> UserInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> McpServerInfra for LucardRepo<F>
where
...
#[async_trait::async_trait]
impl<F> CommandInfra for LucardRepo<F>
where
...
impl<F: FileInfoInfra + EnvironmentInfra + DirectoryReaderInfra + Send + Sync> AgentRepository
    for LucardRepo<F>
{
    async fn get_agents(&self) -> anyhow::Result<Vec<lucard_domain::AgentDefinition>> {
...
impl<F: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra + Send + Sync>
    SkillRepository for LucardRepo<F>
{
...
impl<F: StrategyFactory> StrategyFactory for LucardRepo<F> {
    type Strategy = F::Strategy;

    fn create_auth_strategy(
        &self,
        provider_id: ProviderId,
        auth_method: lucard_domain::AuthMethod,
        required_params: Vec<lucard_domain::URLParam>,
