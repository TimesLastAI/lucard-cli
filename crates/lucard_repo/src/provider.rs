...
use merge::Merge;
use lucard_app::domain::{ProviderId, ProviderResponse};
use lucard_app::{EnvironmentInfra, FileReaderInfra, FileWriterInfra};
use lucard_domain::{
...
enum Models {
    ...
    Hardcoded(Vec<lucard_app::domain::Model>),
}
...
struct ProviderConfig {
    ...
    #[merge(strategy = merge::vec::append)]
    auth_methods: Vec<lucard_domain::AuthMethod>,
}
...
impl From<&ProviderConfig>
    for Provider<lucard_domain::Template<HashMap<lucard_domain::URLParam, lucard_domain::URLParamValue>>>
{
    fn from(config: &ProviderConfig) -> Self {
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => {
                lucard_domain::ModelSource::Url(lucard_domain::Template::new(model_url_template))
            }
            Models::Hardcoded(model_list) => {
                lucard_domain::ModelSource::Hardcoded(model_list.clone())
            }
        });
        ...
        Provider {
            ...
            url: lucard_domain::Template::new(&config.url),
...
pub struct LucardProviderRepository<F> {
    infra: Arc<F>,
    handlebars: &'static Handlebars<'static>,
}

impl<F> LucardProviderRepository<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self { infra, handlebars: get_handlebars() }
    }
}

impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra> LucardProviderRepository<F> {
    ...
    async fn get_providers(&self) -> Vec<AnyProvider> {
        ...
        for config in configs {
            // Skip Lucard provider as it's handled specially
    ...
    pub async fn migrate_env_to_file(&self) -> anyhow::Result<Option<MigrationResult>> {
        ...
        for config in configs {
            // Skip Lucard provider and ContextEngine providers - they're not configurable
    ...
    async fn create_provider(&self, config: &ProviderConfig) -> anyhow::Result<Provider<Url>> {
        ...
        let models = config.models.as_ref().map(|m| match m {
            Models::Url(model_url_template) => {
                ...
                .unwrap();
                lucard_domain::ModelSource::Url(model_url)
            }
            Models::Hardcoded(model_list) => {
                lucard_domain::ModelSource::Hardcoded(model_list.clone())
            }
    ...
    fn create_unconfigured_provider(
        ...
        Provider<lucard_domain::Template<HashMap<lucard_domain::URLParam, lucard_domain::URLParamValue>>>,
    > {
        Ok(config.into())
    }
    ...
    async fn provider_from_id(&self, id: ProviderId) -> anyhow::Result<Provider<Url>> {
        ...
        if id == ProviderId::FORGE {
            // Lucard provider isn't typically configured via env vars in the registry
...
impl<F: EnvironmentInfra + FileReaderInfra + FileWriterInfra + Sync> ProviderRepository
    for LucardProviderRepository<F>
{
...
#[cfg(test)]
mod tests {
    use lucard_app::domain::ProviderResponse;
...
mod env_tests {
    ...
    impl FileReaderInfra for MockInfra {
        ...
        async fn range_read_utf8(
            ...
            _start_line: u64,
            _end_line: u64,
        ) -> anyhow::Result<(String, lucard_domain::FileInfo)> {
            Err(anyhow::anyhow!("File not found"))
        }
    }
    ...
    impl ProviderRepository for MockInfra {
        ...
        async fn upsert_credential(
            &self,
            _credential: lucard_domain::AuthCredential,
        ) -> anyhow::Result<()> {
            Ok(())
        }
        ...
        async fn get_credential(
            &self,
            _id: &ProviderId,
        ) -> anyhow::Result<Option<lucard_domain::AuthCredential>> {
            Ok(None)
        }
        ...
        async fn migrate_env_credentials(
            &self,
        ) -> anyhow::Result<Option<lucard_domain::MigrationResult>> {
            Ok(None)
        }
    }
    ...
    async fn test_migration_from_env_to_file() {
        ...
        let registry = LucardProviderRepository::new(infra.clone());
    ...
    #[tokio::test]
    async fn test_migration_should_not_create_lucard_services_credential() {
        let mut env_vars = HashMap::new();
        ...
        // Verify lucard_services was NOT created during migration
    ...
    async fn test_create_azure_provider_with_handlebars_urls() {
        ...
        let infra = Arc::new(MockInfra::new(env_vars));
        let registry = LucardProviderRepository::new(infra);
        ...
        assert_eq!(
            provider
                ...
                .and_then(|c| match &c.auth_details {
                    lucard_domain::AuthDetails::ApiKey(key) => Some(key.to_string()),
                    _ => None,
                }),
        ...
        match &provider.models.as_ref().unwrap() {
            ...
            lucard_domain::ModelSource::Hardcoded(_) => panic!("Expected Models::Url variant"),
        }
    }
