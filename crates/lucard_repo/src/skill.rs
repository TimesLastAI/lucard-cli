...
/// - **Global skills**: `~/lucard/skills/<skill-name>/SKILL.md`
/// - **CWD skills**: `./.lucard/skills/<skill-name>/SKILL.md` (relative to
///   current working directory)
///
/// Missing directories are handled gracefully and don't prevent loading from
/// other sources.
pub struct LucardSkillRepository<I> {
    infra: Arc<I>,
}

impl<I> LucardSkillRepository<I> {
    ...
    fn load_builtin_skills(&self) -> Vec<Skill> {
        let builtin_skills = vec![
            (
                "lucard://skills/create-skill/SKILL.md",
                include_str!("skills/create-skill/SKILL.md"),
            ),
            (
                "lucard://skills/execute-plan/SKILL.md",
...
impl<I: FileInfoInfra + EnvironmentInfra + FileReaderInfra + WalkerInfra> LucardSkillRepository<I> {
    ...
    fn render_skill(&self, skill: Skill, env: &lucard_domain::Environment) -> anyhow::Result<Skill> {
        ...
        let rendered_command = TemplateEngine::default()
            .render_template(lucard_domain::Template::new(&skill.command), &skill_context)?;
...
#[cfg(test)]
mod tests {
    use lucard_infra::LucardInfra;
    use pretty_assertions::assert_eq;

    use super::*;

    fn fixture_skill_repo() -> (LucardSkillRepository<LucardInfra>, std::path::PathBuf) {
        let skill_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/fixtures/skills_with_resources");
        let infra = Arc::new(LucardInfra::new(false, std::env::current_dir().unwrap()));
        let repo = LucardSkillRepository::new(infra);
        (repo, skill_dir)
    }
    ...
    fn test_load_builtin_skills() {
        // Fixture
        let repo = LucardSkillRepository { infra: Arc::new(()) };
        ...
        assert_eq!(
            create_skill.path,
            Some(std::path::Path::new("lucard://skills/create-skill/SKILL.md").to_path_buf())
        );
        assert_eq!(
        ...
        assert_eq!(
            execute_plan.path,
            Some(std::path::Path::new("lucard://skills/execute-plan/SKILL.md").to_path_buf())
        );
        assert!(execute_plan
            .description
    ...
    async fn test_extract_skill_with_valid_metadata() {
        ...
        let content =
            lucard_common::fixture!("/src/fixtures/skills/with_name_and_description.md").await;
    ...
    async fn test_extract_skill_with_incomplete_metadata() {
        // Fixture
        let content = lucard_common::fixture!("/src/fixtures/skills/with_name_only.md").await;
