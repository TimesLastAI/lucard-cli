...
use lucard_app::AppConfigService;
use lucard_app::domain::Provider;
use lucard_domain::{AppConfig, AppConfigRepository, ModelId, ProviderId, ProviderRepository};
use url::Url;

/// Service for managing user preferences for default providers and models.
pub struct LucardAppConfigService<F> {
    infra: Arc<F>,
}

impl<F> LucardAppConfigService<F> {
...
impl<F: ProviderRepository + AppConfigRepository> LucardAppConfigService<F> {
    ...
    async fn get_provider_model(
        ...
        let provider_id = match provider_id {
            ...
            None => config
                .provider
                .as_ref()
                .ok_or(lucard_domain::Error::NoDefaultProvider)?,
        };
        ...
        Ok(config
            ...
            .get(provider_id)
            .cloned()
            .ok_or_else(|| lucard_domain::Error::no_default_model(provider_id.clone()))?)
    }
...
mod tests {
    ...
    async fn test_get_default_provider_when_none_set() -> anyhow::Result<()> {
        ...
        let service = LucardAppConfigService::new(Arc::new(fixture));
    ...
    async fn test_get_default_provider_when_set() -> anyhow::Result<()> {
        let fixture = MockInfra::new();
        let service = LucardAppConfigService::new(Arc::new(fixture.clone()));
