...
use anyhow::bail;
use bytes::Bytes;
use lucard_app::{AuthService, EnvironmentInfra, Error, HttpInfra, User, UserUsage};
use lucard_domain::{AppConfigRepository, InitAuth, LoginInfo};
use reqwest::Url;
...
pub struct LucardAuthService<I> {
    infra: Arc<I>,
}

impl<I: HttpInfra + EnvironmentInfra + AppConfigRepository> LucardAuthService<I> {
    pub fn new(infra: Arc<I>) -> Self {
        Self { infra }
    }
    async fn init(&self) -> anyhow::Result<InitAuth> {
        let init_url = format!("{}{AUTH_ROUTE}", self.infra.get_environment().lucard_api_url);
    ...
    async fn login(&self, auth: &InitAuth) -> anyhow::Result<LoginInfo> {
        let url = format!(
            "{}{AUTH_ROUTE}{}",
            self.infra.get_environment().lucard_api_url,
            auth.session_id
        );
...
impl<I: HttpInfra + EnvironmentInfra + AppConfigRepository> AuthService for LucardAuthService<I> {
