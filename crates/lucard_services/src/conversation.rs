...
use anyhow::Result;
use lucard_app::ConversationService;
use lucard_app::domain::{Conversation, ConversationId};
use lucard_domain::ConversationRepository;

/// Service for managing conversations, including creation, retrieval, and
/// updates
#[derive(Clone)]
pub struct LucardConversationService<S> {
    conversation_repository: Arc<S>,
}

impl<S: ConversationRepository> LucardConversationService<S> {
    /// Creates a new LucardConversationService with the provided repository
...
impl<S: ConversationRepository> ConversationService for LucardConversationService<S> {
        ...
        T: Send,
    {
        let mut conversation = self
            ...
            .get_conversation(id)
            .await?
            .ok_or_else(|| lucard_app::domain::Error::ConversationNotFound(*id))?;
