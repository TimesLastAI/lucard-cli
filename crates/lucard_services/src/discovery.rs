...
use anyhow::Result;
use lucard_app::domain::File;
use lucard_app::{DirectoryReaderInfra, EnvironmentInfra, FileDiscoveryService, Walker, WalkerInfra};

pub struct LucardDiscoveryService<F> {
    service: Arc<F>,
}

impl<F> LucardDiscoveryService<F> {
    pub fn new(service: Arc<F>) -> Self {
        Self { service }
    }
}

impl<F: EnvironmentInfra + WalkerInfra> LucardDiscoveryService<F> {
...
mod tests {
    ...
    async fn test_list_current_directory_sorts_dirs_first() {
        ...
        let service = LucardDiscoveryService::new(Arc::new(fixture));
