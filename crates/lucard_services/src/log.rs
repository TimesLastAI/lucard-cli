...
pub fn init_tracing(log_path: PathBuf) -> anyhow::Result<Guard> {
    ...
    // Create a filter that only allows logs from lucard_ modules
    let filter = filter::filter_fn(|metadata| metadata.target().starts_with("lucard_"));
...
fn prepare_writer(
    ...
    let append = tracing_appender::rolling::daily(log_path, "lucard.log");
    let (non_blocking, guard) = tracing_appender::non_blocking(append);
    let env = tracing_subscriber::EnvFilter::new("lucard=debug");
