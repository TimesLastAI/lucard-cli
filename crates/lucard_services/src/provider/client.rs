        ...
        let inner = match response_type {
            ...
            ProviderResponse::Anthropic => {
                ...
                let creds = provider
                    ...
                    .auth_details;
                match creds {
                    lucard_domain::AuthDetails::ApiKey(api_key) => {
                    ...
                    lucard_domain::AuthDetails::OAuth { tokens, .. } => {
                        InnerClient::Anthropic(Box::new(Anthropic::new(
                            http.clone(),
...
pub struct Client<T: lucard_app::HttpClientService> {
    retry_config: Arc<RetryConfig>,
    inner: Arc<InnerClient<T>>,
    models_cache: Arc<RwLock<HashMap<ModelId, Model>>>,
}

impl<T: lucard_app::HttpClientService> Clone for Client<T> {
    fn clone(&self) -> Self {
        Self {
...
impl<T: lucard_app::HttpClientService> Client<T> {
...
mod tests {
    ...
    use bytes::Bytes;
    use lucard_app::HttpClientService;
    use lucard_app::domain::ProviderId;
    use reqwest::Url;
    use reqwest::header::HeaderMap;
    ...
    fn make_test_credential() -> Option<lucard_domain::AuthCredential> {
    ...
    async fn test_cache_initialization() {
        let provider = lucard_domain::Provider {
            id: ProviderId::OPENAI,
