...
use lucard_app::HttpClientService;
use lucard_app::domain::{
    ChatCompletionMessage, Context as ChatContext, ModelId, ProviderId, ResultStream, Transformer,
};
use lucard_app::dto::openai::{ListModelResponse, ProviderPipeline, Request, Response};
use lucard_domain::Provider;
...
impl<H: HttpClientService> OpenAIProvider<H> {
    ...
    fn get_headers(&self) -> Vec<(String, String)> {
        ...
        if let Some(api_key) = self
            ...
            .map(|c| match &c.auth_details {
                lucard_domain::AuthDetails::ApiKey(key) => key.as_str(),
                lucard_domain::AuthDetails::OAuthWithApiKey { api_key, .. } => api_key.as_str(),
                lucard_domain::AuthDetails::OAuth { tokens, .. } => tokens.access_token.as_str(),
            })
        {
        ...
        self.provider
            ...
            .for_each(|method| match method {
                lucard_domain::AuthMethod::ApiKey => {}
                lucard_domain::AuthMethod::OAuthDevice(oauth_config) => {
                ...
                lucard_domain::AuthMethod::OAuthCode(oauth_config) => {
                    if let Some(custom_headers) = &oauth_config.custom_headers {
                        custom_headers.iter().for_each(|(k, v)| {
                            headers.push((k.clone(), v.clone()));
                        });
    ...
    async fn inner_models(&self) -> Result<Vec<lucard_app::domain::Model>> {
        ...
        if self.provider.id == ProviderId::VERTEX_AI {
            ...
            match models {
                lucard_domain::ModelSource::Url(url) => {
                ...
                lucard_domain::ModelSource::Hardcoded(models) => {
    ...
    fn inner_vertex_models(&self) -> Vec<lucard_app::domain::Model> {
        lazy_static! {
            static ref VERTEX_MODELS: Vec<lucard_app::domain::Model> = {
                let models =
...
impl<T: HttpClientService> OpenAIProvider<T> {
    ...
    pub async fn models(&self) -> Result<Vec<lucard_app::domain::Model>> {
...
mod tests {
    ...
    use bytes::Bytes;
    use lucard_app::HttpClientService;
    use lucard_app::domain::{Provider, ProviderId, ProviderResponse};
    ...
    fn make_credential(provider_id: ProviderId, key: &str) -> Option<lucard_domain::AuthCredential> {
        Some(lucard_domain::AuthCredential {
            id: provider_id,
            auth_details: lucard_domain::AuthDetails::ApiKey(lucard_domain::ApiKey::from(
                key.to_string(),
            )),
    ...
    fn openai(key: &str) -> Provider<Url> {
        Provider {
            ...
            url_params: vec![],
            models: Some(lucard_domain::ModelSource::Url(
