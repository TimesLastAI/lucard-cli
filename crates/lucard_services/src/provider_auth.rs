...
use lucard_app::{AuthStrategy, ProviderAuthService, StrategyFactory};
use lucard_domain::{
    AuthContextRequest, AuthContextResponse, AuthMethod, Provider, ProviderId, ProviderRepository,
};

/// Lucard Provider Authentication Service
#[derive(Clone)]
pub struct LucardProviderAuthService<I> {
    infra: Arc<I>,
}

impl<I> LucardProviderAuthService<I> {
...
impl<I> ProviderAuthService for LucardProviderAuthService<I>
where
    ...
    async fn init_provider_auth(
        ...
        let required_params = if matches!(auth_method, AuthMethod::ApiKey) {
            ...
            let provider = providers
                ...
                .ok_or_else(|| lucard_domain::Error::provider_not_available(provider_id.clone()))?;
                        ...
                        AuthMethod::OAuthDevice(_) | AuthMethod::OAuthCode(_) => {
                            ...
                            let existing_credential =
                                self.infra.get_credential(&provider.id).await?.ok_or_else(
                                    || lucard_domain::Error::ProviderNotAvailable {
