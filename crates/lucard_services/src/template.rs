...
use lucard_app::{EnvironmentInfra, FileReaderInfra, TemplateService};
use lucard_domain::Template;
use tokio::sync::RwLock;

#[derive(Clone)]
pub struct LucardTemplateService<F> {
    hb: Arc<RwLock<Handlebars<'static>>>,
    infra: Arc<F>,
}

impl<F: EnvironmentInfra + FileReaderInfra> LucardTemplateService<F> {
    pub fn new(infra: Arc<F>) -> Self {
        let hb = lucard_app::TemplateEngine::handlebar_instance();
...
impl<F: EnvironmentInfra + FileReaderInfra> TemplateService for LucardTemplateService<F> {
...
mod tests {
    ...
    async fn test_render_simple_template() {
        ...
        let service = LucardTemplateService::new(Arc::new(MockCompositeService::new()));
        let data = json!({
            "name": "Lucard",
            "version": "1.0",
        ...
        let expected = "App: Lucard v1.0 - Features: templates, rendering, handlebars";
    ...
    async fn test_render_partial_system_info() {
        ...
        let actual = service
            .render_template(Template::new("{{> lucard-partial-system-info.md }}"), &data)
            .await
            .unwrap();
