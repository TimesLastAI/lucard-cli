...
use lucard_domain::{FileInfo, Image};
...
pub struct LucardFsRead<F>(Arc<F>);

impl<F> LucardFsRead<F> {
...
impl<F: FileInfoInfra + EnvironmentInfra + InfraFsReadService> FsReadService for LucardFsRead<F> {
    async fn read(
        &self,
        path: String,
        ...
        if is_visual_content(&mime_type) {
            ...
            assert_file_size(&*self.0, path, env.max_image_size).await.with_context(|| {
                if mime_type == "application/pdf" {
                    "PDF exceeds size limit. Use a smaller PDF or increase LUCARD_MAX_IMAGE_SIZE."
                } else {
