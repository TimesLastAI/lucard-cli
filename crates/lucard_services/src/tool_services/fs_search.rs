...
// Using FSSearchInput from lucard_domain
...
/// subsequent access.
pub struct LucardFsSearch<W> {
    infra: Arc<W>,
}

impl<W> LucardFsSearch<W> {
...
impl<W: WalkerInfra + FileReaderInfra + FileInfoInfra> FsSearchService for LucardFsSearch<W> {
    async fn search(
        &self,
        input_path: String,
...
impl<W: WalkerInfra + FileInfoInfra> LucardFsSearch<W> {
...
mod test {
    ...
    use lucard_app::{WalkedFile, Walker};
    use lucard_domain::FileInfo;
    use tokio::fs;
    ...
    async fn test_search_content_with_regex() {
        ...
        let actual = LucardFsSearch::new(Arc::new(MockInfra::default()))
            .search(
    ...
    async fn test_search_nonexistent_path() {
        let result = LucardFsSearch::new(Arc::new(MockInfra::default()))
            .search(
