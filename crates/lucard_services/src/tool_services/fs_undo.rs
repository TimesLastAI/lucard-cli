...
use lucard_app::{FileInfoInfra, FileReaderInfra, FsUndoOutput, FsUndoService};
...
#[derive(Default)]
pub struct LucardFsUndo<F> {
    infra: Arc<F>,
}

impl<F> LucardFsUndo<F> {
...
impl<F: FileInfoInfra + FileReaderInfra + SnapshotRepository> FsUndoService for LucardFsUndo<F> {
