...
use anyhow::Context;
use lucard_app::domain::Image;
use lucard_app::{EnvironmentInfra, FileInfoInfra, ImageReadService};
use strum_macros::{Display, EnumString};

use crate::utils::assert_absolute_path;

pub struct LucardImageRead<F>(Arc<F>);
...
impl<F> LucardImageRead<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self(infra)
    }
}
#[async_trait::async_trait]
impl<F: FileInfoInfra + EnvironmentInfra + lucard_app::FileReaderInfra> ImageReadService
    for LucardImageRead<F>
{
    async fn read_image(&self, path: String) -> anyhow::Result<Image> {
        ...
        crate::tool_services::fs_read::assert_file_size(&*self.0, path, env.max_image_size)
            .await
            .with_context(
                || "Image exceeds size limit. Compress the image or increase LUCARD_MAX_IMAGE_SIZE.",
            )?;
