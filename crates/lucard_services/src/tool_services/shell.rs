...
use lucard_app::{CommandInfra, EnvironmentInfra, ShellOutput, ShellService};
...
/// unrestricted access, advise users to run lucard CLI with '-u' flag. Returns
/// complete output including stdout, stderr, and exit code for diagnostic
/// purposes.
pub struct LucardShell<I> {
    env: Environment,
    infra: Arc<I>,
}

impl<I: EnvironmentInfra> LucardShell<I> {
...
impl<I: CommandInfra + EnvironmentInfra> ShellService for LucardShell<I> {
    async fn execute(
        &self,
        command: String,
        cwd: PathBuf,
...
mod tests {
    ...
    use lucard_app::domain::{CommandOutput, Environment};
    use lucard_app::{CommandInfra, ShellService};
    ...
    async fn test_shell_service_forwards_env_vars() {
        let fixture = LucardShell::new(Arc::new(MockCommandInfra {
    ...
    async fn test_shell_service_forwards_no_env_vars() {
        let fixture = LucardShell::new(Arc::new(MockCommandInfra { expected_env_vars: None }));
