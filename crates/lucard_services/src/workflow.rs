...
use anyhow::Context;
use lucard_app::domain::Workflow;
use lucard_app::{FileReaderInfra, FileWriterInfra, WorkflowService};

/// A workflow loader to load the workflow from the given path.
/// It also resolves the internal paths specified in the workflow.
pub struct LucardWorkflowService<F> {
    infra: Arc<F>,
}

impl<F> LucardWorkflowService<F> {
    pub fn new(infra: Arc<F>) -> Self {
        Self { infra }
    }
}

impl<F: FileWriterInfra + FileReaderInfra> LucardWorkflowService<F> {
    /// Find a lucard.yaml config file by traversing parent directories.
    ...
    pub async fn resolve_path(&self, path: Option<PathBuf>) -> PathBuf {
        ...
        if path.exists() || path.to_string_lossy() != "lucard.yaml" {
    ...
    /// If the path is just "lucard.yaml", searches for it in parent directories.
...
impl<F: FileWriterInfra + FileReaderInfra> WorkflowService for LucardWorkflowService<F> {
    ...
    async fn read_workflow(&self, path: Option<&Path>) -> anyhow::Result<Workflow> {
        let path_to_use = path.unwrap_or_else(|| Path::new("lucard.yaml"));
...
mod tests {
    ...
    fn test_find_config_file_behavior() {
        ...
        let config_path = temp_dir.path().join("lucard.yaml");
        fs::write(&config_path, "test content").unwrap();

        let result = find_config_file_logic(Path::new("lucard.yaml"), &config_path);
        assert_eq!(result, config_path);

        // Test 2: Return original path for non-lucard.yaml files
        ...
        let parent_config = parent_dir.join("lucard.yaml");
        fs::write(&parent_config, "parent config").unwrap();

        let result = find_config_file_logic(Path::new("lucard.yaml"), &parent_config);
    ...
    fn test_explicit_path_not_searched() {
        ...
        // Create lucard.yaml in the parent
        fs::write(parent_dir.join("lucard.yaml"), "# Test").unwrap();

        // Simulate search with a non-lucard.yaml path
        let custom_path = PathBuf::from("custom-config.yaml");
        let parent_config = parent_dir.join("lucard.yaml");
